...
uint64_t exception_handler_c(registers_t *regs) {
    ...
    if (is_user_mode_fault) {
        ...
        serial_write(buf);
        serial_write("\nRAX: 0x");
        itoa_hex(regs->rax, buf);
        serial_write(buf);
        serial_write("\nRBX: 0x");
        itoa_hex(regs->rbx, buf);
        serial_write(buf);
        serial_write("\nRCX: 0x");
        itoa_hex(regs->rcx, buf);
        serial_write(buf);
        serial_write("\nRDX: 0x");
        itoa_hex(regs->rdx, buf);
        serial_write(buf);
        serial_write("\nRSI: 0x");
        itoa_hex(regs->rsi, buf);
        serial_write(buf);
        serial_write("\nRDI: 0x");
        itoa_hex(regs->rdi, buf);
        serial_write(buf);
        serial_write("\nRSP: 0x");
        itoa_hex(regs->rsp, buf);
        serial_write(buf);
        serial_write("\nRBP: 0x");
        itoa_hex(regs->rbp, buf);
        serial_write(buf);
        serial_write("\nR8:  0x");
        itoa_hex(regs->r8, buf);
        serial_write(buf);
        serial_write("\nR9:  0x");
        itoa_hex(regs->r9, buf);
        serial_write(buf);
        serial_write("\nR10: 0x");
        itoa_hex(regs->r10, buf);
        serial_write(buf);
        serial_write("\nR11: 0x");
        itoa_hex(regs->r11, buf);
        serial_write(buf);
        serial_write("\nR12: 0x");
        itoa_hex(regs->r12, buf);
        serial_write(buf);
        serial_write("\nR13: 0x");
        itoa_hex(regs->r13, buf);
        serial_write(buf);
        serial_write("\nR14: 0x");
        itoa_hex(regs->r14, buf);
        serial_write(buf);
        serial_write("\nR15: 0x");
