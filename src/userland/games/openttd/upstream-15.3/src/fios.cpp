...
#if defined(BOREDOS)
	struct stat st;
	fios->mtime = stat(filename.c_str(), &st) == 0 ? static_cast<int64_t>(st.st_mtime) * 1000 : 0;
#else
...
static void FiosGetFileList(SaveLoadOperation fop, bool show_dirs, FiosGetTypeAndNameProc *callback_proc, Subdirectory subdir, FileList &file_list)
{
	...
	if (show_dirs) {
		...
		int count = sys_list(_fios_path->c_str(), entries, lengthof(entries));
		if (count > 0) {
			for (int i = 0; i < count; i++) {
				...
				if (name.starts_with(".") && name != PERSONAL_DIR) continue;

				FiosItem &fios = file_list.emplace_back();
				fios.type = FIOS_TYPE_DIR;
				fios.mtime = 0;
				fios.name = std::string{name};
