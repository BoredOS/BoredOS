...
#if defined(BOREDOS)
			std::string_view text{sign->name};
			if (text.empty() || toupper(text.front()) != 'T') continue;

			size_t pos = 1;
			while (pos < text.size() && std::isspace(static_cast<unsigned char>(text[pos]))) pos++;

			size_t index_start = pos;
			while (pos < text.size() && std::isdigit(static_cast<unsigned char>(text[pos]))) pos++;
			if (index_start == pos) continue;

			while (pos < text.size() && std::isspace(static_cast<unsigned char>(text[pos]))) pos++;
			size_t flags_start = pos;

			size_t end = text.size();
			while (end > flags_start && std::isspace(static_cast<unsigned char>(text[end - 1]))) end--;
			size_t delay_start = end;
			while (delay_start > flags_start && std::isdigit(static_cast<unsigned char>(text[delay_start - 1]))) delay_start--;
			if (delay_start == end) continue;

			size_t flags_end = delay_start;
			while (flags_end > flags_start && std::isspace(static_cast<unsigned char>(text[flags_end - 1]))) flags_end--;
			if (flags_start == flags_end) continue;

			std::string_view index_text = text.substr(index_start, pos - index_start);
			std::string_view flags = text.substr(flags_start, flags_end - flags_start);
			std::string_view delay_text = text.substr(delay_start, end - delay_start);
#else
			std::smatch match;
...
#if defined(BOREDOS)
			if (auto value = ParseInteger<int>(index_text); value.has_value()) {
#else
...
#if defined(BOREDOS)
			if (auto value = ParseInteger<uint>(delay_text); value.has_value()) {
#else
