...
std::string NewGRFProfiler::GetOutputFilename() const
{
	...
	const int year = tm != nullptr ? tm->tm_year + 1900 : 1970;
	const int month = tm != nullptr ? tm->tm_mon + 1 : 1;
	const int day = tm != nullptr ? tm->tm_mday : 1;
	const int hour = tm != nullptr ? tm->tm_hour : 0;
	const int minute = tm != nullptr ? tm->tm_min : 0;
	return fmt::format("{}grfprofile-{:04d}{:02d}{:02d}-{:02d}{:02d}-{:08X}.csv", FiosGetScreenshotDir(), year, month, day, hour, minute, std::byteswap(this->grffile->grfid));
#else
