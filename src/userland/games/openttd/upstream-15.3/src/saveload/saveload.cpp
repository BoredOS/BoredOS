...
static inline void BoredOSSerialWrite(const char *str)
{
	syscall2(8, 0, (uint64_t)str);
}

static jmp_buf _boredos_saveload_error_jmp;
static bool _boredos_saveload_error_active = false;
...
#if defined(BOREDOS)
	BoredOSSerialWrite("[OpenTTD NET] save/load error\n");
	if (_boredos_saveload_error_active) longjmp(_boredos_saveload_error_jmp, 1);
#endif
...
{
#if defined(BOREDOS)
	_boredos_saveload_error_active = true;
	if (setjmp(_boredos_saveload_error_jmp) != 0) {
		_boredos_saveload_error_active = false;
		ClearSaveLoadState();
		return SL_REINIT;
	}
#endif
	try {
		_sl.action = SLA_LOAD;
		SaveOrLoadResult result = DoLoad(std::move(reader), false);
#if defined(BOREDOS)
		_boredos_saveload_error_active = false;
#endif
		return result;
