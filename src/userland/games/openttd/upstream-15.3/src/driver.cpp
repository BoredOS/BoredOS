...
bool DriverFactoryBase::SelectDriverImpl(const std::string &name, Driver::Type type)
{
	...
	if (name.empty()) {
		...
		size_t driver_name_end = name.find(':');
		std::string dname = name.substr(0, driver_name_end);

		std::vector<std::string> parms;
		if (driver_name_end != std::string::npos) {
			size_t param_start = driver_name_end + 1;
			while (param_start <= name.size()) {
				size_t param_end = name.find(',', param_start);
				parms.push_back(name.substr(param_start, param_end == std::string::npos ? std::string::npos : param_end - param_start));
