...
#define BOREDOS_LANG_LOG(message) sys_serial_write(message)
#else
#define BOREDOS_LANG_LOG(message) do { } while (false)
#endif
#include <stack>
...
#if defined(BOREDOS)
static std::string BoredOSLanguageFilename(const LanguageMetadata *lang)
{
	std::string path = FS2OTTD(lang->file.native());
...
#if defined(BOREDOS)
	_config_language_file = BoredOSLanguageFilename(_current_language);
#else
...
{
#if defined(BOREDOS)
	BOREDOS_LANG_LOG("[OpenTTD] FillLanguageList begin\n");
	FAT32_FileInfo entries[128];
	int count = sys_list(path.c_str(), entries, lengthof(entries));
	if (count < 0) {
		BOREDOS_LANG_LOG("[OpenTTD] FillLanguageList sys_list failed\n");
		Debug(misc, 9, "Unable to open directory {}", path);
		return;
	}
	BOREDOS_LANG_LOG("[OpenTTD] FillLanguageList sys_list done\n");

	for (int i = 0; i < count; i++) {
		if (entries[i].is_directory || entries[i].name[0] == '\0') continue;

		std::string_view name{entries[i].name};
		if (name.size() < 4 || name.substr(name.size() - 4) != ".lng") continue;

		LanguageMetadata lmd;
		std::string file = path;
		AppendPathSeparator(file);
	...
	BOREDOS_LANG_LOG("[OpenTTD] FillLanguageList done\n");
#else
...
{
	BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks begin\n");
	for (Searchpath sp : _valid_searchpaths) {
		FillLanguageList(FioGetDirectory(sp, LANG_DIR));
	}
	BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks scan complete\n");
	if (_languages.empty()) {
		BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks no languages\n");
		UserError("No available language packs (invalid versions?)");
	}

	/* Acquire the locale of the current system */
	BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks locale begin\n");
	auto str_lang = GetCurrentLocale("LC_MESSAGES");
	std::string_view lang = str_lang.has_value() ? std::string_view{*str_lang} : "en_GB";
	BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks locale done\n");
	...
	BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks choose begin\n");
	for (const LanguageMetadata &lng : _languages) {
		/* We are trying to find a default language. The priority is by
		 * configuration file, local environment and last, if nothing found,
		 * English. */
#if defined(BOREDOS)
		if (_config_language_file == BoredOSLanguageFilename(&lng)) {
#else
	...
	BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks choose done\n");

	BOREDOS_LANG_LOG("[OpenTTD] ReadLanguagePack begin\n");
	if (!ReadLanguagePack(chosen_language)) {
		BOREDOS_LANG_LOG("[OpenTTD] ReadLanguagePack failed\n");
		UserError("Can't read language pack '{}'", FS2OTTD(chosen_language->file.native()));
	}
	BOREDOS_LANG_LOG("[OpenTTD] InitializeLanguagePacks done\n");
}
