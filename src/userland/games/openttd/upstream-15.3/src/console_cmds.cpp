...
{
	size_t start = 0;
	while (start < full_string.size()) {
		size_t end = full_string.find('\n', start);
		if (end == std::string::npos) end = full_string.size();
		size_t trimmed_end = end;
		if (trimmed_end > start && full_string[trimmed_end - 1] == '\r') trimmed_end--;
		IConsolePrint(CC_DEFAULT, full_string.substr(start, trimmed_end - start));
		start = end + 1;
	}
}
...
static bool ConGetSysDate(std::span<std::string_view> argv)
{
	...
	if (tm == nullptr) {
		IConsolePrint(CC_ERROR, "System Date: unavailable");
		return true;
	}

	IConsolePrint(CC_DEFAULT, "System Date: {:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}",
