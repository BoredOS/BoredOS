...
#if defined(BOREDOS)
	NWidgetScrollbar *scrollbar = (nw->type == NWID_HSCROLLBAR || nw->type == NWID_VSCROLLBAR) ? static_cast<NWidgetScrollbar *>(nw) : nullptr;
#else
...
#if defined(BOREDOS)
	NWidgetBase *base_child = this->children.front().get();
	NWidgetCore *child = (base_child != nullptr && ((base_child->type & WWT_MASK) < WWT_LAST || (base_child->type & WWT_MASK) == NWID_BUTTON_DROPDOWN || base_child->type == NWID_HSCROLLBAR || base_child->type == NWID_VSCROLLBAR || base_child->type == NWID_VIEWPORT)) ? static_cast<NWidgetCore *>(base_child) : nullptr;
#else
...
#if defined(BOREDOS)
/*
 * BoredOS does not have a complete C++ ABI implementation for dynamic_cast yet.
 * The freestanding __dynamic_cast shim is intentionally conservative elsewhere,
 * but widget tree construction needs exact downcasts or it corrupts object
 * layouts. Use the existing WidgetType tag for the nested widget hierarchy.
 */
static bool IsCoreWidgetType(WidgetType tp)
{
	return (tp & WWT_MASK) < WWT_LAST ||
		(tp & WWT_MASK) == NWID_BUTTON_DROPDOWN ||
		tp == NWID_HSCROLLBAR ||
		tp == NWID_VSCROLLBAR ||
		tp == NWID_VIEWPORT;
}

static bool IsResizeBaseWidgetType(WidgetType tp)
{
	return IsCoreWidgetType(tp) || tp == NWID_SPACER;
}

static bool IsPIPContainerWidgetType(WidgetType tp)
{
	return tp == NWID_HORIZONTAL ||
		tp == NWID_HORIZONTAL_LTR ||
		tp == NWID_VERTICAL ||
		tp == NWID_MATRIX;
}

static bool IsBackgroundWidgetType(WidgetType tp)
{
	return tp == WWT_PANEL || tp == WWT_FRAME || tp == WWT_INSET;
}

static bool IsPlainContainerWidgetType(WidgetType tp)
{
	return IsPIPContainerWidgetType(tp) || tp == NWID_SELECTION || tp == NWID_LAYER;
}

static NWidgetResizeBase *BoredOSCastResizeBase(NWidgetBase *wid)
{
	return wid != nullptr && IsResizeBaseWidgetType(wid->type) ? static_cast<NWidgetResizeBase *>(wid) : nullptr;
}

static NWidgetCore *BoredOSCastCore(NWidgetBase *wid)
{
	return wid != nullptr && IsCoreWidgetType(wid->type) ? static_cast<NWidgetCore *>(wid) : nullptr;
}

static NWidgetPIPContainer *BoredOSCastPIPContainer(NWidgetBase *wid)
{
	return wid != nullptr && IsPIPContainerWidgetType(wid->type) ? static_cast<NWidgetPIPContainer *>(wid) : nullptr;
}

static NWidgetBackground *BoredOSCastBackground(NWidgetBase *wid)
{
	return wid != nullptr && IsBackgroundWidgetType(wid->type) ? static_cast<NWidgetBackground *>(wid) : nullptr;
}

static NWidgetContainer *BoredOSCastContainer(NWidgetBase *wid)
{
	return wid != nullptr && IsPlainContainerWidgetType(wid->type) ? static_cast<NWidgetContainer *>(wid) : nullptr;
}

static NWidgetHorizontal *BoredOSCastHorizontal(NWidgetBase *wid)
{
	return wid != nullptr && (wid->type == NWID_HORIZONTAL || wid->type == NWID_HORIZONTAL_LTR) ? static_cast<NWidgetHorizontal *>(wid) : nullptr;
}
#endif
...
void ApplyNWidgetPartAttribute(const NWidgetPart &nwid, NWidgetBase *dest)
{
	switch (nwid.type) {
		case WPT_RESIZE: {
#if defined(BOREDOS)
			NWidgetResizeBase *nwrb = BoredOSCastResizeBase(dest);
#else
		...
		case WPT_TEXTSTYLE: {
#if defined(BOREDOS)
			NWidgetCore *nwc = BoredOSCastCore(dest);
#else
		...
		case WPT_PIPSPACE: {
#if defined(BOREDOS)
			NWidgetPIPContainer *nwc = BoredOSCastPIPContainer(dest);
			NWidgetBackground *nwb = BoredOSCastBackground(dest);
#else
...
#if defined(BOREDOS)
	NWidgetContainer *nwid_cont = BoredOSCastContainer(parent.get());
	NWidgetBackground *nwid_parent = BoredOSCastBackground(parent.get());
#else
...
#if defined(BOREDOS)
	NWidgetHorizontal *hor_cont = BoredOSCastHorizontal(nwid.get());
#else
