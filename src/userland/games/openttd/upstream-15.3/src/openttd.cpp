...
{
#if defined(BOREDOS)
	sys_serial_write("[OpenTTD] UserError: ");
	sys_serial_write(str.c_str());
	sys_serial_write("\n");
#endif
...
{
#if defined(BOREDOS)
	sys_serial_write("[OpenTTD] FatalError: ");
	sys_serial_write(str.c_str());
	sys_serial_write("\n");
#endif
...
{
#if defined(BOREDOS)
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame enter\n");
#endif
	_game_mode = GM_MENU;
	...
	/* Setup main window */
#if defined(BOREDOS)
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame reset windows begin\n");
#endif
	ResetWindowSystem();
	SetupColoursAndInitialWindow();
#if defined(BOREDOS)
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame reset windows done\n");
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame title load begin\n");
#endif

	/* Load the default opening screen savegame */
#if defined(BOREDOS)
	/*
	 * The title-game save/load path still depends on hosted savegame support
	 * that is not needed for first-boot playability. Keep the generated empty
	 * intro world and bring up the real main menu.
	 */
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame title load skipped on BoredOS\n");
...
#if defined(BOREDOS)
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame finalise begin\n");
#endif
	FixTitleGameZoom();
	_pause_mode = {};
...
#if defined(BOREDOS)
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame done\n");
#endif
}
...
int openttd_main(std::span<std::string_view> arguments)
{
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] DeterminePaths begin\n");
	DeterminePaths(arguments[0], only_local_path);
	BOREDOS_STARTUP_LOG("[OpenTTD] DeterminePaths done\n");
	BOREDOS_STARTUP_LOG("[OpenTTD] baseset tar scan begin\n");
	TarScanner::DoScan(TarScanner::Mode::Baseset);
	BOREDOS_STARTUP_LOG("[OpenTTD] baseset tar scan done\n");
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadFromConfig begin\n");
	LoadFromConfig(true);
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadFromConfig done\n");
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] language scan begin\n");
	InitializeLanguagePacks();
	BOREDOS_STARTUP_LOG("[OpenTTD] language scan done\n");

	/* Initialize the font cache */
	BOREDOS_STARTUP_LOG("[OpenTTD] font cache begin\n");
	FontCache::LoadFontCaches(FONTSIZES_REQUIRED);
	BOREDOS_STARTUP_LOG("[OpenTTD] font cache done\n");

	/* This must be done early, since functions use the SetWindowDirty* calls */
	InitWindowSystem();

	BOREDOS_STARTUP_LOG("[OpenTTD] BaseGraphics::FindSets begin\n");
	BaseGraphics::FindSets();
	BOREDOS_STARTUP_LOG("[OpenTTD] BaseGraphics::FindSets done\n");
...
#ifdef BOREDOS
		valid_graphics_set = BaseGraphics::SetSetByName("OpenGFX");
		if (!valid_graphics_set) BaseGraphics::SetSet(nullptr);
#else
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] palette/blitter begin\n");
	Debug(misc, 1, "Loading blitter...");
	if (blitter.empty() && !_ini_blitter.empty()) blitter = _ini_blitter;
#if defined(BOREDOS)
	/* BoredOS currently validates the plain framebuffer path only. Treat this
	 * as a fixed blitter choice so the generic startup/NewGRF probe code cannot
	 * switch back to animated or host-accelerated blitters.
	 */
	blitter = "32bpp-simple";
#endif
	 ...
	 */
	const std::string_view default_32bpp_blitter =
#if defined(BOREDOS)
			"32bpp-simple";
#else
			"32bpp-anim";
	...
	if (!_blitter_autodetected ||
			...
			BlitterFactory::SelectBlitter(default_32bpp_blitter) == nullptr) {
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] palette/blitter done\n");
	if (videodriver.empty() && !_ini_videodriver.empty()) videodriver = _ini_videodriver;
	BOREDOS_STARTUP_LOG("[OpenTTD] selecting video driver\n");
	DriverFactoryBase::SelectDriver(videodriver, Driver::DT_VIDEO);
	BOREDOS_STARTUP_LOG("[OpenTTD] video driver selected\n");
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] social/network/bootstrap begin\n");
	SocialIntegration::Initialize();
	NetworkStartUp(); // initialize network-core

	if (!HandleBootstrap()) {
		BOREDOS_STARTUP_LOG("[OpenTTD] bootstrap failed\n");
		ShutdownGame();
		return 0;
	}
	BOREDOS_STARTUP_LOG("[OpenTTD] bootstrap done\n");

	VideoDriver::GetInstance()->ClaimMousePointer();

	BOREDOS_STARTUP_LOG("[OpenTTD] BaseSounds::FindSets begin\n");
	BaseSounds::FindSets();
	BOREDOS_STARTUP_LOG("[OpenTTD] BaseSounds::FindSets done\n");
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] BaseMusic::FindSets begin\n");
	BaseMusic::FindSets();
	BOREDOS_STARTUP_LOG("[OpenTTD] BaseMusic::FindSets done\n");
	...
	BOREDOS_STARTUP_LOG("[OpenTTD] world/menu setup begin\n");
	BOREDOS_STARTUP_LOG("[OpenTTD] GenerateWorld intro begin\n");
	GenerateWorld(GWM_EMPTY, 64, 64); // Make the viewport initialization happy
	BOREDOS_STARTUP_LOG("[OpenTTD] GenerateWorld intro done\n");
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame(false) begin\n");
	LoadIntroGame(false);
	BOREDOS_STARTUP_LOG("[OpenTTD] LoadIntroGame(false) done\n");

	/* ScanNewGRFFiles now has control over the scanner. */
	BOREDOS_STARTUP_LOG("[OpenTTD] RequestNewGRFScan begin\n");
	RequestNewGRFScan(scanner.release());
	BOREDOS_STARTUP_LOG("[OpenTTD] RequestNewGRFScan done\n");

	BOREDOS_STARTUP_LOG("[OpenTTD] entering video main loop\n");
...
#if defined(BOREDOS)
static void BoredOSCapMapSize()
{
	if (_settings_game.game_creation.map_x > 8) _settings_game.game_creation.map_x = 8;
	if (_settings_game.game_creation.map_y > 8) _settings_game.game_creation.map_y = 8;
}
#endif
...
static void MakeNewGame(bool from_heightmap, bool reset_settings)
{
	...
	#if defined(BOREDOS)
		BoredOSCapMapSize();
		BOREDOS_STARTUP_LOG("[OpenTTD] BoredOS capped new-game map generation at 256x256\n");
	#endif
...
static void MakeNewEditorWorld()
{
	...
	#if defined(BOREDOS)
		BoredOSCapMapSize();
		BOREDOS_STARTUP_LOG("[OpenTTD] BoredOS capped editor map generation at 256x256\n");
	#endif
...
void SwitchToMode(SwitchMode new_mode)
{
	...
	switch (new_mode) {
		...
		case SM_GENRANDLAND: // Generate random land within scenario editor
			...
			#if defined(BOREDOS)
				BoredOSCapMapSize();
				BOREDOS_STARTUP_LOG("[OpenTTD] BoredOS capped random-land generation at 256x256\n");
			#endif
