...
{
#if defined(BOREDOS)
	return sys_exists(std::string{filename}.c_str()) != 0;
#else
	std::error_code ec;
...
std::optional<FileHandle> FioFOpenFile(std::string_view filename, std::string_view mode, Subdirectory subdir, size_t *filesize)
{
	...
	if (!f.has_value() && mode[0] == 'r' && subdir != NO_DIRECTORY) {
		...
		while (token_start <= resolved_name.size()) {
			size_t token_end = resolved_name.find(PATHSEPCHAR, token_start);
			std::string token = resolved_name.substr(token_start, token_end == std::string::npos ? std::string::npos : token_end - token_start);
			if (token == "..") {
...
void FioCreateDirectory(const std::string &name)
{
	...
	for (char c : name) {
		path.push_back(c);
		if (c != PATHSEPCHAR || path == PATHSEP) continue;
		if (path.size() > 1) mkdir(path.c_str(), 0755);
	}
	if (!path.empty() && path != PATHSEP) mkdir(path.c_str(), 0755);
#else
...
{
#if defined(BOREDOS)
	if (sys_delete(filename.c_str()) != 0) {
		Debug(misc, 0, "Removing {} failed", filename);
		return false;
	}
	return true;
#else
...
#if defined(BOREDOS)
	size_t filename_pos = filename.find_last_of(PATHSEPCHAR);
	filename_base = filename_pos == std::string::npos ? filename : filename.substr(filename_pos + 1);
#else
	filename_base = FS2OTTD(std::filesystem::path(OTTD2FS(filename)).filename().native());
#endif
...
#if defined(BOREDOS)
			size_t end = _config_file.find_last_of(PATHSEPCHAR);
			tmp = end == std::string::npos ? std::string{} : _config_file.substr(0, end);
#else
 ...
 */
static uint ScanPath(FileScanner *fs, std::string_view extension,
#if defined(BOREDOS)
	const std::string &path,
#else
	const std::filesystem::path &path,
#endif
	size_t basepath_length, bool recursive)
{
	uint num = 0;
	...
	if (count < 0) {
		Debug(misc, 9, "Unable to read directory {}", path);
		return 0;
	}

	for (int i = 0; i < count; i++) {
		if (entries[i].name[0] == '\0') continue;
		std::string_view entry_name{entries[i].name};
		...
		if (entries[i].is_directory) {
			...
			num += ScanPath(fs, extension, file, basepath_length, recursive);
		} else {
...
#if defined(BOREDOS)
		num += ScanPath(this, extension, path, path.size(), recursive);
#else
...
#if defined(BOREDOS)
	return ScanPath(this, extension, path, path.size(), recursive);
#else
