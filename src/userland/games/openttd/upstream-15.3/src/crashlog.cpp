...
#if defined(BOREDOS)
static std::string FormatCrashTimestamp(bool filename)
{
	time_t now = time(nullptr);
	struct tm *tm = gmtime(&now);
	if (tm == nullptr) return filename ? "crash00000000000000" : "0000-00-00 00:00:00 (UTC)";

	if (filename) {
		return fmt::format("crash{:04d}{:02d}{:02d}{:02d}{:02d}{:02d}",
			tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	}

	return fmt::format("{:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d} (UTC)",
		tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
}
#endif
...
std::string CrashLog::CreateFileName(std::string_view ext, bool with_dir) const
{
	...
	if (crashname.empty()) {
#if defined(BOREDOS)
		crashname = FormatCrashTimestamp(true);
#else
...
#if defined(BOREDOS)
	this->survey["date"] = FormatCrashTimestamp(false);
#else
