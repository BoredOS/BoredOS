...
bool TCPConnecter::CheckActivity()
{
	...
	switch (this->status) {
		case Status::Init:
#if defined(BOREDOS)
			/* BoredOS userland threading is not ready for OpenTTD's resolver worker yet.
			 * Run the resolver on the game thread so coordinator/listing connects advance.
			 */
			BOREDOS_CONNECTER_LOG("[OpenTTD NET] resolving synchronously\n");
			this->Resolve();
			break;
#else
