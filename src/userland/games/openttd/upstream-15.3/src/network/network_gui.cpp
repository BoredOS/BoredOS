...
#ifdef BOREDOS
			/* On BoredOS, Refresh also acts as a coordinator refresh when the list is empty. */
			this->SetWidgetDisabledState(WID_NG_REFRESH, false);
#else
		...
		switch (widget) {
			...
			case WID_NG_JOIN: // Join Game
				...
				case WID_NG_REFRESH: // Refresh
					...
					this->searched_internet = true;
#else
					if (this->server != nullptr && !this->server->refreshing) {
...
[[maybe_unused]] static void ShowNetworkStartServerWindow()
{
