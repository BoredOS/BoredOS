...
static bool BoredOSConnectionStringLooksIPv6(std::string_view connection_string)
{
	...
	if (connection_string.front() == '[') return true;

	size_t first_colon = connection_string.find(':');
	if (first_colon == std::string_view::npos) return false;
	return connection_string.find(':', first_colon + 1) != std::string_view::npos;
}
#else
...
public:
	...
	void OnFailure() override
	{
		BOREDOS_COORDINATOR_LOG("[OpenTTD NET] coordinator connect failed\n");
	...
	void OnConnect(SOCKET s) override
	{
		BOREDOS_COORDINATOR_LOG("[OpenTTD NET] coordinator connected\n");
...
{
	static bool boredos_logged_listing = false;
	uint16_t servers = p.Recv_uint16();
	Debug(net, 9, "Coordinator::Receive_GC_LISTING({})", servers);
	if (!boredos_logged_listing) {
		BOREDOS_COORDINATOR_LOG("[OpenTTD NET] receiving coordinator listing\n");
		boredos_logged_listing = true;
	}

	/* End of list; we can now remove all expired items from the list. */
	if (servers == 0) {
		BOREDOS_COORDINATOR_LOG("[OpenTTD NET] coordinator listing complete\n");
		boredos_logged_listing = false;
...
#if defined(BOREDOS)
		if (BoredOSConnectionStringLooksIPv6(connection_string)) {
			NetworkGameInfo skipped_info{};
			DeserializeNetworkGameInfo(p, skipped_info, &this->newgrf_lookup_table);
			ClearGRFConfigList(skipped_info.grfconfig);
			BOREDOS_COORDINATOR_LOG("[OpenTTD NET] skipped IPv6-only coordinator server\n");
			continue;
		}
#endif
...
void ClientNetworkCoordinatorSocketHandler::Connect()
{
	...
	BOREDOS_COORDINATOR_LOG("[OpenTTD NET] coordinator connect begin\n");
	this->Reopen();
...
{
	BOREDOS_COORDINATOR_LOG("[OpenTTD NET] requesting server listing\n");
	this->Connect();
