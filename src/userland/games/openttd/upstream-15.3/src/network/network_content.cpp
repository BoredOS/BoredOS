...
void ClientNetworkContentSocketHandler::DownloadSelectedContent(uint &files, uint &bytes, bool fallback)
{
	...
	if (fallback) {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] selected download fallback begin\n");
		this->DownloadSelectedContentFallback(content);
	} else {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] selected download HTTP begin\n");
...
{
	BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] HTTP metadata request begin\n");
...
{
	BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] fallback protocol request begin\n");
...
static bool GunzipFile(const ContentInfo &ci)
{
#if defined(WITH_ZLIB)
#if defined(BOREDOS)
	BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] gunzip begin\n");

	auto ftmp = FileHandle::Open(GetFullFilename(ci, true), "rb");
	auto fout = FileHandle::Open(GetFullFilename(ci, false), "wb");
	if (!ftmp.has_value() || !fout.has_value()) {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] gunzip open failed\n");
		return false;
	}

	z_stream stream{};
	if (inflateInit2(&stream, 15 + 32) != Z_OK) {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] gunzip init failed\n");
		return false;
	}
	...
	while (!done) {
		if (stream.avail_in == 0 && !input_eof) {
			size_t read = fread(in, 1, sizeof(in), *ftmp);
			if (read == 0) {
				input_eof = true;
			} else {
				stream.next_in = in;
		...
		do {
			...
			int zret = inflate(&stream, input_eof ? Z_FINISH : Z_NO_FLUSH);
			...
			if (have != 0 && fwrite(out, 1, have, *fout) != have) {
				ret = false;
				done = true;
			}

			if (zret == Z_STREAM_END) break;
			if (stream.avail_out != 0 && stream.avail_in == 0) break;
	...
	if (ret) {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] gunzip complete\n");
	} else {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] gunzip failed\n");
	}
	return ret;
#else
	bool ret = true;
...
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] install begin\n");
#endif
		FioRemove(GetFullFilename(*this->cur_info, true));
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] compressed file removed\n");
#endif

		Subdirectory sd = GetContentInfoSubDir(this->cur_info->type);
		if (sd == NO_DIRECTORY) {
#if defined(BOREDOS)
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] install failed: no content directory\n");
...
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] tar scan begin\n");
#endif
		if (!ts.AddFile(sd, fname)) {
#if defined(BOREDOS)
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] tar scan failed\n");
#endif
			ShowErrorMessage(GetEncodedString(STR_CONTENT_ERROR_COULD_NOT_EXTRACT), {}, WL_ERROR);
			return;
		}
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] tar scan complete\n");
#endif

		if (this->cur_info->type == CONTENT_TYPE_BASE_MUSIC) {
			/* Music can't be in a tar. So extract the tar! */
#if defined(BOREDOS)
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] base music extract begin\n");
#endif
			if (!ExtractTar(fname, BASESET_DIR)) {
#if defined(BOREDOS)
				BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] base music extract failed\n");
#endif
				ShowErrorMessage(GetEncodedString(STR_CONTENT_ERROR_COULD_NOT_EXTRACT), {}, WL_ERROR);
				return;
			}
			FioRemove(fname);
#if defined(BOREDOS)
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] base music extract complete\n");
#endif
		}
...
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] completion callbacks begin\n");
#endif
		this->OnDownloadComplete(this->cur_info->id);
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] completion callbacks done\n");
#endif
	} else {
		ShowErrorMessage(GetEncodedString(STR_CONTENT_ERROR_COULD_NOT_EXTRACT), {}, WL_ERROR);
	}
...
{
	BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] download failure\n");
...
void ClientNetworkContentSocketHandler::OnReceiveData(std::unique_ptr<char[]> data, size_t length)
{
	...
	if (this->http_response_index == -1) {
		if (data != nullptr) {
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] HTTP metadata chunk received\n");
			/* Append the rest of the response. */
			this->http_response.insert(this->http_response.end(), data.get(), data.get() + length);
			return;
		} else {
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] HTTP metadata complete\n");
	...
	if (data != nullptr) {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] HTTP file chunk received\n");
	...
	if (this->cur_file.has_value()) {
		...
		finished_file = true;
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] HTTP download complete\n");
		this->http_response.clear();
		this->http_response.shrink_to_fit();
		this->http_response_index = -2;
		return;
...
#if defined(BOREDOS)
		if (finished_file) {
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] HTTP downloads complete\n");
			this->http_response.clear();
			this->http_response.shrink_to_fit();
			this->http_response_index = -2;
			return;
...
public:
	...
	{
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] content socket connect failed\n");
	...
	{
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] content socket connected\n");
...
void ClientNetworkContentSocketHandler::Connect()
{
	...
	if (!sys_network_is_initialized() || !sys_network_has_ip()) {
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] waiting for network\n");
		if (sys_network_init() != 0 || !sys_network_has_ip()) {
			BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] network not ready\n");
			return;
		}
	}
#endif
	...
	BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] content socket connect begin\n");
...
void ClientNetworkContentSocketHandler::OnDownloadComplete(ContentID cid)
{
	...
	for (size_t i = 0; i < this->callbacks.size(); /* nothing */) {
		...
		/* the callback may remove itself from this->callbacks */
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] completion callback begin\n");
#endif
		cb->OnDownloadComplete(cid);
#if defined(BOREDOS)
		BOREDOS_CONTENT_LOG("[OpenTTD CONTENT] completion callback done\n");
#endif
		if (i != this->callbacks.size() && this->callbacks[i] == cb) i++;
	}
}
