...
static bool BoredOSClientProxySendAll(int fd, const char *data, size_t len)
{
	return BoredOSClientProxySendAll(fd, reinterpret_cast<const uint8_t *>(data), len);
}

static int BoredOSClientProxyRecvSome(int fd, uint8_t *data, size_t len)
{
...
static bool BoredOSClientProxyRecvLine(int fd, char *line, size_t line_len)
{
	if (line_len == 0) return false;
	size_t pos = 0;
	while (pos + 1 < line_len) {
		uint8_t ch = 0;
		int ret = BoredOSClientProxyRecvSome(fd, &ch, 1);
...
static void BoredOSClientLogSize(const char *prefix, size_t value, const char *suffix)
{
	char buf[96];
	size_t pos = 0;
	...
	buf[pos] = '\0';
	BOREDOS_CLIENT_LOG(buf);
}

static bool BoredOSClientProxyConnect(int *out_fd)
{
	...
	*out_fd = -1;
	...
	if (!sys_network_is_initialized() || !sys_network_has_ip()) {
		if (sys_network_init() != 0 || !sys_network_has_ip()) return false;
	}
...
static void BoredOSClientWriteSizeLine(char *out, size_t out_len, size_t value)
{
	if (out_len < 2) return;
	char tmp[24];
	size_t pos = 0;
...
static bool BoredOSDecompressNetworkSavegame(struct PacketReader *reader);
#else
...
#if defined(BOREDOS)
static bool BoredOSDecompressNetworkSavegame(PacketReader *reader)
{
	if (reader == nullptr || reader->buffer.size() < 8) return true;

	auto it = reader->buffer.cbegin();
	uint8_t tag0 = *it++;
	uint8_t tag1 = *it++;
	uint8_t tag2 = *it++;
	uint8_t tag3 = *it++;
	if (tag0 != 'O' || tag1 != 'T' || tag2 != 'T' || tag3 != 'X') return true;

	BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma map decompress begin\n");

	std::vector<uint8_t> compressed;
	compressed.reserve(reader->buffer.size());
	std::ranges::copy(reader->buffer, std::back_inserter(compressed));

	int fd = -1;
	if (!BoredOSClientProxyConnect(&fd)) {
		BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy connect failed\n");
		return false;
	}
	BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy connected\n");

	char size_line[32];
	BoredOSClientWriteSizeLine(size_line, sizeof(size_line), compressed.size());

	bool ok = BoredOSClientProxySendAll(fd, "OTHP1\n", 6) &&
		BoredOSClientProxySendAll(fd, "LZMA_DECOMPRESS\n", 16) &&
		BoredOSClientProxySendAll(fd, "local://savegame\n", 17) &&
		BoredOSClientProxySendAll(fd, size_line, std::char_traits<char>::length(size_line)) &&
		BoredOSClientProxySendAll(fd, compressed.data(), compressed.size());
	if (!ok) {
		sys_socket_close(fd);
		BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy send failed\n");
		return false;
	}
	BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy request sent\n");

	char line[64];
	if (!BoredOSClientProxyRecvLine(fd, line, sizeof(line))) {
		sys_socket_close(fd);
		BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy response failed\n");
		return false;
	}
	BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy response header received\n");

	if (line[0] != 'O' || line[1] != 'K' || line[2] != ' ') {
		sys_socket_close(fd);
		BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy returned error\n");
		return false;
	}

	size_t total = 0;
	for (const char *p = line + 3; *p >= '0' && *p <= '9'; p++) total = (total * 10U) + (size_t)(*p - '0');
	if (total < 8 || total > 128U * 1024U * 1024U) {
		sys_socket_close(fd);
		BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy size invalid\n");
		return false;
	}
	BoredOSClientLogSize("[OpenTTD NET] lzma proxy response size ", total, "\n");

	std::vector<uint8_t> decompressed(total);
	size_t got = 0;
	size_t next_log = 256U * 1024U;
	while (got < total) {
		size_t want = std::min<size_t>(total - got, 4096);
		int ret = BoredOSClientProxyRecvSome(fd, decompressed.data() + got, want);
		if (ret <= 0) {
			sys_socket_close(fd);
			BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma proxy body failed\n");
			return false;
		}
		got += (size_t)ret;
		if (got >= next_log || got == total) {
			BoredOSClientLogSize("[OpenTTD NET] lzma proxy body received ", got, "\n");
			next_log += 256U * 1024U;
		}
	}
	...
	sys_socket_close(fd);
	reader->buffer.clear();
	std::ranges::copy(decompressed, std::back_inserter(reader->buffer));
	reader->Reset();
	BOREDOS_CLIENT_LOG("[OpenTTD NET] lzma map decompress done\n");
	return true;
}
#endif
...
NetworkRecvStatus ClientNetworkGameSocketHandler::Receive_SERVER_MAP_BEGIN(Packet &p)
{
	...
	BOREDOS_CLIENT_LOG("[OpenTTD NET] map download begin\n");
...
NetworkRecvStatus ClientNetworkGameSocketHandler::Receive_SERVER_MAP_SIZE(Packet &p)
{
	...
	BOREDOS_CLIENT_LOG("[OpenTTD NET] map download size received\n");
	SetWindowDirty(WC_NETWORK_STATUS_WINDOW, WN_NETWORK_STATUS_WINDOW_JOIN);
...
NetworkRecvStatus ClientNetworkGameSocketHandler::Receive_SERVER_MAP_DONE(Packet &)
{
	...
	BOREDOS_CLIENT_LOG("[OpenTTD NET] map download done, loading\n");
...
#if defined(BOREDOS)
	if (!BoredOSDecompressNetworkSavegame(this->savegame.get())) {
		this->savegame = nullptr;
		BOREDOS_CLIENT_LOG("[OpenTTD NET] map decompress failed\n");
		return NETWORK_RECV_STATUS_SERVER_ERROR;
	}
#endif
	...
	this->savegame = nullptr;
	BOREDOS_CLIENT_LOG(load_success ? "[OpenTTD NET] map load ok\n" : "[OpenTTD NET] map load failed\n");
