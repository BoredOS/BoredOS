...
#ifdef BOREDOS
	/* The public coordinator listing already includes NetworkGameInfo. BoredOS'
	 * TCP layer is currently stable enough for coordinator and join sockets, but
	 * per-row query sockets can still tear down the process while probing public
	 * servers. Avoid that redundant path and rely on coordinator refreshes.
	 */
	NetworkGameListAddItem(connection_string)->refreshing = false;
	UpdateNetworkGameWindow();
	return;
#endif
