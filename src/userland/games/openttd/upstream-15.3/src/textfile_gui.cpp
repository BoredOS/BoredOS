...
void TextfileWindow::FindHyperlinksInMarkdown(Line &line, size_t line_index)
{
	...
	while (search_pos < line.text.size()) {
		size_t text_begin = line.text.find('[', search_pos);
		if (text_begin == std::string::npos) break;

		size_t text_end = line.text.find(']', text_begin + 1);
		if (text_end == std::string::npos) break;
		if (text_end == text_begin + 1 || text_end + 1 >= line.text.size() || line.text[text_end + 1] != '(') {
			search_pos = text_begin + 1;
			continue;
		}

		size_t destination_begin = text_end + 2;
		size_t destination_end = line.text.find(')', destination_begin);
		if (destination_end == std::string::npos) break;
		if (destination_end == destination_begin) {
			search_pos = destination_begin + 1;
			continue;
		}

		Hyperlink &link = this->links.emplace_back(line_index, 0, 0, line.text.substr(destination_begin, destination_end - destination_begin));
		...
		if (link_colour != SCC_CONTROL_END) {
			builder += std::string_view(line.text).substr(last_match_end, text_begin - last_match_end);
			link.begin = fixed_line.length();
			builder.PutUtf8(SCC_PUSH_COLOUR);
			builder.PutUtf8(link_colour);
			builder += std::string_view(line.text).substr(text_begin + 1, text_end - text_begin - 1);
			link.end = fixed_line.length();
			builder.PutUtf8(SCC_POP_COLOUR);
			last_match_end = destination_end + 1;
			found = true;
		}

		search_pos = destination_end + 1;
	}
	if (!found) return; // nothing found

	/* Add remaining text on line. */
	fixed_line += line.text.substr(last_match_end);
