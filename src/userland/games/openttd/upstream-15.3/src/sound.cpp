...
static void StartSound(SoundID sound_id, float pan, uint volume)
{
	...
	if (now - last_beep_tick >= 4) {
		int freq = 392 + (int)(sound_id % 12) * 31;
		if (sound_id == SND_15_BEEP) freq = 880;
		if (sound_id == SND_14_CASHTILL) freq = 1175;
		if (sound_id == SND_1F_CONSTRUCTION_OTHER || sound_id == SND_20_CONSTRUCTION_RAIL) freq = 523;
		if (sound_id == SND_12_EXPLOSION || sound_id == SND_13_TRAIN_COLLISION) freq = 131;
		int ms = sound_id == SND_15_BEEP ? 35 : 70;
		sys_system(SYSTEM_CMD_BEEP, freq, ms, 0, 0);
		last_beep_tick = now;
	}
#endif
