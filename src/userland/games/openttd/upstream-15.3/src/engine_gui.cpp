...
static std::string GetTrainEngineInfoString(const Engine &e)
{
	...
	res.append(GetString(STR_ENGINE_PREVIEW_COST_WEIGHT, e.GetCost(), e.GetDisplayWeight()));
	res.push_back('\n');
	...
	if (e.VehInfo<RailVehicleInfo>().railtypes.Count() > 1) {
		...
		res.append(GetString(STR_ENGINE_PREVIEW_RAILTYPES, railtypes));
		res.push_back('\n');
	}
	...
	if (_settings_game.vehicle.train_acceleration_model != AM_ORIGINAL && !is_maglev) {
		res.append(GetString(STR_ENGINE_PREVIEW_SPEED_POWER_MAX_TE, PackVelocity(e.GetDisplayMaxSpeed(), e.type), e.GetPower(), e.GetDisplayMaxTractiveEffort()));
		res.push_back('\n');
	} else {
		res.append(GetString(STR_ENGINE_PREVIEW_SPEED_POWER, PackVelocity(e.GetDisplayMaxSpeed(), e.type), e.GetPower()));
		res.push_back('\n');
	}

	res.append(GetPreviewRunningCostString(e));
	res.push_back('\n');

	uint capacity = GetTotalCapacityOfArticulatedParts(e.index);
	res.append(GetString(STR_ENGINE_PREVIEW_CAPACITY, capacity == 0 ? INVALID_CARGO : e.GetDefaultCargoType(), capacity));
...
static std::string GetAircraftEngineInfoString(const Engine &e)
{
	...
	res.append(GetString(STR_ENGINE_PREVIEW_COST_MAX_SPEED, e.GetCost(), PackVelocity(e.GetDisplayMaxSpeed(), e.type)));
	res.push_back('\n');

	if (uint16_t range = e.GetRange(); range > 0) {
		res.append(GetString(STR_ENGINE_PREVIEW_TYPE_RANGE, e.GetAircraftTypeText(), range));
		res.push_back('\n');
	} else {
		res.append(GetString(STR_ENGINE_PREVIEW_TYPE, e.GetAircraftTypeText()));
		res.push_back('\n');
	}
	...
	if (mail_capacity > 0) {
		res.append(GetString(STR_ENGINE_PREVIEW_CAPACITY_2, cargo, capacity, GetCargoTypeByLabel(CT_MAIL), mail_capacity));
	} else {
		res.append(GetString(STR_ENGINE_PREVIEW_CAPACITY, cargo, capacity));
	}
...
static std::string GetShipEngineInfoString(const Engine &e)
{
	...
	res.append(GetString(STR_ENGINE_PREVIEW_CAPACITY, e.GetDefaultCargoType(), e.GetDisplayDefaultCapacity()));
